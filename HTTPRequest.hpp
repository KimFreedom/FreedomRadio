...
namespace http
{
    ...
    public:
        ...
        Response send_server_request(const std::string& method = "GET",
            const std::string& body = "",
            const std::vector<std::string>& headers = {})
        {
            return send_server_request(method,
                std::vector<uint8_t>(body.begin(), body.end()),
                headers);
        }

        Response send_server_request(const std::string& method,
            const std::vector<uint8_t>& body,
            const std::vector<std::string>& headers)
        {
            Response response;
            ...
            // read the response
            std::ofstream outfile("StreamData.txt", std::ofstream::binary);
            for (int iPacket = 0; iPacket < 100; iPacket++)
            {
                const auto size = socket.recv(tempBuffer, sizeof(tempBuffer), noSignal);
                ...
                outfile.write(reinterpret_cast<const char*>(tempBuffer), size);
            }

            return response;
        }

        Socket send_and_get_socket(const std::string& method = "GET",
            const std::string& body = "",
            const std::vector<std::string>& headers = {})
        {
            return send_and_get_socket(method,
                std::vector<uint8_t>(body.begin(), body.end()),
                headers);
        }

        Socket send_and_get_socket(const std::string& method,
            const std::vector<uint8_t>& body,
            const std::vector<std::string>& headers)
        {
            if (scheme != "http")
